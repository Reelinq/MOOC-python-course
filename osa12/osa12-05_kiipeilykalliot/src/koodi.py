...
def reittien_maaran_mukaan(kalliot: list):
	def reittimaarajarjestelma(kallio: Kiipeilykallio):
		return kallio.reitteja()
	return sorted(kalliot, key=reittimaarajarjestelma)

def vaikeimman_reitin_mukaan(kalliot: list):
	def vaikeammanreitinjarejestelma(kallio: Kiipeilykallio):
		return kallio.vaikein_reitti().grade
	return sorted(kalliot, key=vaikeammanreitinjarejestelma, reverse=True)